...
const popFlash = () => {
  const v = sessionStorage.getItem("flash_msg");
  if (v) sessionStorage.removeItem("flash_msg");
  return v || undefined;
};
...
const Login: React.FC = () => {
  ...
  const successMsg = useMemo<string | undefined>(
    () => location?.state?.msg || popFlash(),
    [location?.state?.msg]
  );
  ...
  const onSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      if (!user) return setServerErr("Email hoặc mật khẩu không đúng");
      ...
      sessionStorage.setItem(
        "authUser",
        JSON.stringify({ id: user.id, email: user.email, role: admin ? "admin" : "user" })
      );
      // thông báo toàn app / các tab
      window.dispatchEvent(new Event("auth:changed"));

      navigate(admin ? "/ManagerUser" : "/BlogPage", { replace: true });
    } catch {
      ...
      <div className="flex flex-1 items-center justify-center px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 w-full max-w-6xl items-center">
          ...
          <div className="w-full max-w-md">
            ...
            {successMsg && (
              <div className="mb-3 rounded-md bg-green-50 text-green-700 px-3 py-2 text-sm">{successMsg}</div>
            )}
            {serverErr && (
              <div className="mb-3 rounded-md bg-red-50 text-red-700 px-3 py-2 text-sm">{serverErr}</div>
            )}
