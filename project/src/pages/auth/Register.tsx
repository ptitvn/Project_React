...
const toHandle = (fullName: string, email: string) =>
  "@" +
...
const Register: React.FC = () => {
  ...
  const validate = async () => {
    ...
    if (!firstName.trim()) e.firstName = "Họ không được để trống";
    if (!lastName.trim())  e.lastName  = "Tên không được để trống";
  ...
  const onSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      await createMember({
        name: fullName || email,
        handle: toHandle(fullName, email),
        email,
        status: "hoạt động",
      ...
      navigate("/login", { state: { msg: "Đăng ký thành công! Vui lòng đăng nhập." }, replace: true });
    } catch {
    ...
    <div className="min-h-screen relative flex items-center justify-center bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#0f172a] overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-12 p-6 md:p-12">
        <div className="flex flex-col items-center justify-center text-center text-white">
          ...
          <p className="mt-6 text-lg text-purple-200 font-semibold">RIKKEI EDUCATION</p>
        </div>
